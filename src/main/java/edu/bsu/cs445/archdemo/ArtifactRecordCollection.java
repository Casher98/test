...
class ArtifactRecordCollection {
    ...
    public List<ArtifactRecord> searchTitles(String query) {
        List<ArtifactRecord> result = items.stream()
                .filter(artifactRecord -> artifactRecord.getTitle().contains(query))
                .collect(Collectors.toList());
        return ImmutableList.copyOf(result);
    }
    public List<ArtifactRecord> searchDates(String query) {
        List<ArtifactRecord> results = items.stream()
                .filter(artifactRecord -> artifactRecord.getDate().contains(query))
                 .collect(Collectors.toList());
